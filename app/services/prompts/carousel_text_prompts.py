...
def build_compaction_prompt(*, lang: str, slide_count: int, source_text_or_draft_slides: Any) -> str:
    return (
        ...
        "body_blocks:[{text, max_lines, role}], bullet_blocks:[{text, max_lines, role}], emphasis_spans, cta}. "
...
def build_user_text_draft_prompt(*, lang: str, slide_count: int, user_text: str) -> str:
    return (
        ...
        "{slide_type, title, body[], bullets[], emphasis_words[], cta}. "
        ...
        "Обычные поясняющие абзацы, которые не являются списком действий, клади в body[], не в bullets[]. "
        "Каждая важная мысль исходного текста должна попасть либо в title, либо в body[], либо в bullets[], либо в cta. "
        "Не оставляй слайд только с title, если в исходном тексте есть контент для распределения. "
        "На каждом слайде должен быть один основной формат содержимого: либо body[], либо bullets[]. "
        "Не заполняй одновременно body[] и bullets[] на одном слайде, кроме редкого случая, когда исходник явно смешивает абзац и список в одном логическом блоке. "
        "Распределяй текст между слайдами сбалансированно: не делай один слайд почти пустым, а другой перегруженным. "
        "Если есть заголовок, затем поясняющий абзац, затем блок вроде 'Что помогает:' со списком — сделай 2 слайда: "
        "первый с title+body, второй с title блока списка и bullets. "
        ...
        "slide_type: первый cover, последний cta или summary, остальные content-like. "
        "Верни только JSON.\n"
